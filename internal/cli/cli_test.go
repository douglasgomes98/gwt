...
func TestHelpListsCommands(t *testing.T) {
	...
	skill := strings.Index(strings.Split(got, "\n")[10], "Install the gwt worktree skill for agents.")
	if got == "" || !bytes.Contains(out.Bytes(), []byte("add <branch>")) || !strings.Contains(got, "init-config") || !strings.Contains(got, "skill install --agents|--claude") || !strings.Contains(got, "upgrade") || add < 0 || skill < 0 || add != skill {
